...
@section('_routes')
    ...
    @if (Auth::user()->user_type == 'admin' || Auth::user()->user_type == 'vet')
        ...
        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
            ...
            <li>
                <a href="{{ route('vaxrecords.index') }}" class="dropdown-item" style="color: #a8a8a8;">Vaccine Records</a>
            </li>
            <li>
            ...
            <li>
                <a href="{{ route('surgicalrecords.index') }}" class="dropdown-item" style="color: #58753C; font-weight: bold;">Surgical Records</a>
            </li>
        </ul>
    @endif
...
@section('layout_content')
    <div class="container-fluid">
        ...
        <div class="row mt-4">
            <p class="display-6">Surgery History</p>
        </div>
        ...
        <div class="row mt-4">
            ...
            <div class="btn-group-sm mt-3" role="group" aria-label="Basic example">
                ...
                @if (count($history) > 0)
                    <a href="{{ route('surgicalhistorydoc.store', $records->id) }}" class="btn btn-secondary btn-sm">
                ...
                <div class="table-responsive">
                    <table class="table bg-light">
                        <tr class="bg-success text-white">
                            <td class="h6" scope="col">Date/Time</td>
                            <td class="h6" scope="col">Weight</td>
                            <td class="h6" scope="col">Procedure</td>
                            <td class="h6" scope="col">PAM</td>
                            <td class="h6" scope="col">AA</td>
                            <td class="h6" scope="col">SAS</td>
                            <td class="h6" scope="col">TechInit</td>
                            <td class="h6" scope="col">AsstInit</td>
                            <td class="h6" scope="col">VetInit</td>
                            <td class="h6" scope="col">Approx. Length</td>
                            <td class="h6" scope="col">Specimens</td>
                            <td class="h6" scope="col">Comments</td>
                        ...
                        @if (count($history) >= 1)
                            @foreach ($history as $history)
                                <tr>
                                    ...
                                    <td>{{ $history->procedure }}</td>
                                    <td>{{ $history->pam }}</td>
                                    <td>{{ $history->aa }}</td>
                                    <td>{{ $history->sas }}</td>
                                    <td>{{ $history->techinit }}</td>
                                    <td>{{ $history->assinit }}</td>
                                    <td>{{ $history->vetinit }}</td>
                                    <td>{{ $history->lengthsurgery }}</td>
                                    <td>{{ $history->specimens }}</td>
                                    <td>{{ $history->comments }}</td>
                                    <td>
                                        <a href="{{ route('surgicalhistory.destroy', $history->id) }}" class="btn" type="button" style="color: #990000;">
                                ...
                                <tr>
                                    <td class="collapse multi-collapse" id="multiCollapseEdit{{ $history->id }}" colspan="100%">
                                        <table class="table mb-0">
                                            <div>
                                                @include('inc.editsurgicalhistory')
                                            </div>
        ...
        <div class="row mt-4">
            <div class="collapse multi-collapse" id="multiCollapseExample1">
                @include('inc.createsurgicalhistory')
            </div>
        </div>
    </div>
@endsection